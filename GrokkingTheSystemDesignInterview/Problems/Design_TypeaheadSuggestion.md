...
Given the amount of data we need to index, we should expect a huge tree. Even, traversing a sub-tree would take really long, e.g., the phrase ‘system design interview questions’ is 30 levels deep. Since we’ve very tight latency requirements, we do need to improve the efficiency of our solution.
...
In addition to a simple count, for terms ranking, we have to consider other factors too, e.
...
### Searches/QPS Estimation
- 5 billion daily searches
=> 60K query/s

- 20% query are unique
- index only top 50% of the search items
- 100 million unique terms to build an index

### Storage Estimation
- avg query words: 3 words
- avg word length: 5 chars
- 1 char = 2 bytes
=> 3 * 5 * 2 = 30 bytes to store an avg query
=> DAU storage: 100 million * 30 bytes = 3GB
